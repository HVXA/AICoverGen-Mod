...
def song_cover_pipeline(
    ...
    try:
        ...
        ai_vocals_path = os.path.join(
            song_dir,
            f'{os.path.splitext(os.path.basename(orig_song_path))[0]}_lead_{voice_model}_p{pitch_change}_i{index_rate}_fr{filter_radius}_rms{rms_mix_rate}_pro{protect}_{f0_method}{"" if f0_method != "mangio-crepe" else f"_{crepe_hop_length}"}.wav',
        )
        ai_backing_path = os.path.join(
            song_dir,
            f'{os.path.splitext(os.path.basename(orig_song_path))[0]}_backing_{voice_model}_p{pitch_change}_i{index_rate}_fr{filter_radius}_rms{rms_mix_rate}_pro{protect}_{f0_method}{"" if f0_method != "mangio-crepe" else f"_{crepe_hop_length}"}.wav',
        )
        ...
        ai_cover_backing_path = os.path.join(
            song_dir,
            f"{os.path.splitext(os.path.basename(orig_song_path))[0]} ({voice_model} Ver With Backing).{output_format}",
        )

        if not os.path.exists(ai_vocals_path):
            display_progress(
                "[~] Converting lead voice using RVC...", 0.5, is_webui, progress
            )
            voice_change(
                voice_model,
                main_vocals_dereverb_path,
            ...
            display_progress(
                "[~] Converting backing voice using RVC...", 0.65, is_webui, progress
            )
            voice_change(
                voice_model,
                backup_vocals_path,
        ...
        ai_backing_mixed_path = add_audio_effects(
            ai_backing_path, reverb_rm_size, reverb_wet, reverb_dry, reverb_damping
        )
        ...
        combine_audio(
            [ai_vocals_mixed_path, ai_backing_mixed_path, instrumentals_path],
            ai_cover_backing_path,
            main_gain,
            backup_gain,
            inst_gain,
            output_format,
