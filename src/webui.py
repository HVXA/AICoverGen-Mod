...
def download_online_model(url, dir_name, progress=gr.Progress()):
    try:
        ...
        if "pixeldrain.com" in url:
            ...
            output = os.path.join(
                ".", f"{dir_name}.zip"
            )  # Adjust the output path if needed
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="AICoverGenWebUI") as app:
        ...
        with gr.Tab("Generate"):
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        pitch = gr.Slider(
                            ...
                            info="Generally, use 12 for male to female conversions and -12 for vice-versa. (Octaves)",
                        )
                        pitch_all = gr.Slider(
                            -12,
                            12,
            ...
            with gr.Row():
                ai_cover = gr.Audio(
                    label="AI Cover (Vocal Only Inference)", show_share_button=False
                )
                ai_backing = gr.Audio(
                    label="AI Cover (Vocal Backing Inference)", show_share_button=False
                )
